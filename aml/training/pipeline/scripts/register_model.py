...
parser.add_argument('--target-col', type=str, dest='target_col', default='Survived', help='target column name')
...
pdf_feature_test = ds_feature_test.to_pandas_dataframe()
pdf_feature_test.iloc[0:10].drop(args.target_col, axis=1).to_csv(os.path.join(model_output_dir, 'sample_data.csv'), index=False)

model = Model.register(workspace=ws, 
                       datasets=[('featurized training data', ds_feature_train), ('featurized test data', ds_feature_test)], 
                       tags={'parent_run_id': parent_id},
